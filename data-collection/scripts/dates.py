...
import calendar  # for date standardization
import pandas as pd
import logging
...
def standardize_date(date: str) -> str:
    ...
    if not date or not isinstance(date, str):
        logger.warning(f"Invalid date input: {date}")
...
def get_date_indexes(column_indexes, target):
    """
    Calculate start and end indexes for date columns based on target period.
    
    Args:
        column_indexes: Dictionary of column information
        target: Target period (e.g., '3 Months', '12 Months')
    
    Returns:
        tuple: (start_idx, end_idx)
    """
    if target not in column_indexes:
        logger.warning(f"Target '{target}' not found in column_indexes")
        return 0, 0
    
    target_columns = ["1 Months", "2 Months", "3 Months", "4 Months", "5 Months", 
...
def get_datetime_index_dates_from_statement(soup: BeautifulSoup, quarterly=False, check_date_indexes=True) -> pd.DatetimeIndex:
    ...
    if not dates:
        logger.warning("No dates found in statement headers")
        return pd.DatetimeIndex([]), []
    
    filtered_indexes = None
    if contain_two_currency:
        if not currencies:
            logger.warning("Multiple currencies detected but none extracted")
        else:
            main_currency = get_most_frequent_currency(currencies)
            date_currencies = currencies
            logger.debug(f"Main currency: {main_currency}")
            logger.debug(f"Date currencies: {date_currencies}")
    ...
    if check_date_indexes:
        ...
        if start_idx == 0 and end_idx == 0:
            logger.warning("Could not determine date indexes, using all dates")
            date_indexes = range(len(dates))
        else:
            date_indexes = range(start_idx, end_idx)
            dates = dates[start_idx:end_idx]
    elif contain_two_currency and filtered_indexes is not None:
        date_indexes = filtered_indexes
    else:
        date_indexes = range(len(dates))
    ...
    try:
        ...
        logger.error(f"Problematic dates: {dates}")
        ...
        dates = pd.DatetimeIndex(valid_dates) if valid_dates else pd.DatetimeIndex([])
